function [ Ak, Bk, HF, Fd, hn, n ] = show_filter_responses_pz( poles, zeros, K, fsample, ...
...
% Convert the poles and zeros into the nummerator & denominator polynomials
Ak = poly(poles);
Bk = K*poly(zeros);
disp(['Filter coefficients of Ak is (',num2str(Ak),')']);
disp(['Filter coefficients of Bk is (',num2str(Bk),')']);
...
elseif (~isempty(indicesGap))
    ...
    cutoffFrequency = [Fd(indicesGap(1)-1), Fd(indicesGap(length(indicesGap))+1)];
    bandwidth = Fd(indicesGap(length(indicesGap))+1) - Fd(indicesGap(1)-1);
end
